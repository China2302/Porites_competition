...
Summarise in terms of Barnacle, Symbiodiniaceae, Coral, Other

```{r}
# Extract metazoa totals
metazoa_data <- mpa_data %>% filter(taxonomy=='d__Eukaryota|k__Metazoa') %>% select(metazoa_rc=read_count,sample)

summary_data <- symbiodinium_data %>% left_join(metazoa_data,by="sample")
